functions{
  ...
  vector cell_population_dynamics(real t,             // Time at which derivatives are evaluated
    ...
    //real alpha_drug = alpha_max + (alpha - alpha_max)*exp(-(drugP/c_P)^b_P)*exp(-(drugF_eff/c_F)^b_F);    
     real alpha_drug = 0.1*alpha_max + (alpha - 0.1*alpha_max)*(1/(1 + (drugP/c_P)^b_P))*(1/(1 + (drugF_eff/c_F)^(0.1*b_F)));
...
//  params[9] = a_FA;
  ...
  params[9] = delta;
     ...
     for (j in 1:3){
        int j_temp;
        j_temp = 8*j - 7;
        y0_cc_pp[j_temp] = y0_cc_p[j]/8;
        y0_cc_pp[j_temp + 1] =  y0_cc_pp[j_temp];
        y0_cc_pp[j_temp + 2] =  y0_cc_pp[j_temp];
        y0_cc_pp[j_temp + 3] =  y0_cc_pp[j_temp];
        y0_cc_pp[j_temp + 4] =  y0_cc_pp[j_temp];
        y0_cc_pp[j_temp + 5] =  y0_cc_pp[j_temp];
        y0_cc_pp[j_temp + 6] =  y0_cc_pp[j_temp];
        y0_cc_pp[j_temp + 7] =  y0_cc_pp[j_temp];
     }
   
   
  for (i in 1:n_drug_combos_cc_p){
    real y_hat_temp[n_obs_ODE_cc,24];
    int x_i[3];
    x_i = {0, 0, 0};
    ...
    y_hat_cc_p[start_obs_ODE_cc[i]:end_obs_ODE_cc[i], 1] = to_vector(y_hat_temp[1:n_obs_ODE_cc, 1]);
      ...
      y_hat_cc_p[start_obs_ODE_cc[i]:end_obs_ODE_cc[i], 2] = to_vector(y_hat_temp[1:n_obs_ODE_cc, 9]);
      for (iter in 1:7){
        y_hat_cc_p[start_obs_ODE_cc[i]:end_obs_ODE_cc[i], 2] = y_hat_cc_p[start_obs_ODE_cc[i]:end_obs_ODE_cc[i], 2] + to_vector(y_hat_temp[1:n_obs_ODE_cc, 9 + iter]);
      }  
      

     y_hat_cc_p[start_obs_ODE_cc[i]:end_obs_ODE_cc[i], 3] = to_vector(y_hat_temp[1:n_obs_ODE_cc, 17]);
      for (iter in 1:7){
        y_hat_cc_p[start_obs_ODE_cc[i]:end_obs_ODE_cc[i], 3] = y_hat_cc_p[start_obs_ODE_cc[i]:end_obs_ODE_cc[i], 3] + to_vector(y_hat_temp[1:n_obs_ODE_cc, 17 + iter]);
      }  
